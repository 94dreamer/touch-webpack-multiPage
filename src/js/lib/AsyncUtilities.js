...
})(window, window['lib'] || (window['lib'] = {}));//viewport
// Copyright (C) 2013:
//    Alex Russell <slightlyoff@chromium.org>
//    Yehuda Katz
//
;(function (browserGlobal, lib) {//Promise
  //
  // Async Utilities
  //
  ...
  var MutationObserver = browserGlobal.MutationObserver ||
    browserGlobal.WebKitMutationObserver; //（变动观察器）是监视DOM变动的接口。
  var Promise;
  ...
  if (MutationObserver) {
    ...
    var observer = new MutationObserver(function () {
      var toProcess = queue.slice();//拷贝一份队列的内容的引用
      queue = [];//队列清空
      toProcess.forEach(function (tuple) {//遍历队列，运行每个队列的办法
        tuple[0].call(tuple[1]);
      });
    });

    var element = document.createElement('div');//监听一个dom 的 attributes变化
    observer.observe(element, {
      attributes: true
    });

    // Chrome Memory Leak: https://bugs.webkit.org/show_bug.cgi?id=93661
    window.addEventListener('unload', function () {/*点击某个离开页面的链接 在地址栏中键入了新的 URL 使用前进或后退按钮 关闭浏览器 重新加载页面*/
...
})(window, window['lib'] || (window['lib'] = {}));//异步

;(function (win, lib, undef) {//navigation
...
})(window, window['lib'] || (window['lib'] = {}));//navigation
...
})(window, window.lib || (window.lib = {}));//一些节点方法 和 dom事件
...
})(window, window['lib'] || (window['lib'] = {}));//运动轨迹
...
})(window, window['lib'] || (window['lib'] = {}))//动画
;
(function (win, lib) {
...
})(window, window['lib'] || (window['lib'] = {}));//贝塞尔曲线
;(function (win, lib, undef) {
  var doc = win.document;
  var ua = win.navigator.userAgent;
  var scrollObjs = {};
...
;(function (win, lib, ctrl) {
  ...
  function Page(name, constructor) {
    ...
    this.startup = this.async(function (root, done) {
      done()
    });
    this.show = this.async(function (root, persisted, done) {
      done()
    });
    this.hide = this.async(function (root, done) {
      done()
    });
    this.teardown = this.async(function (root, done) {
      done()
    });
